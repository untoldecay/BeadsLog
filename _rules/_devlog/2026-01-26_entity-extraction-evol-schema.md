...
### Architectural Relationships
<!-- Format: [From Entity] -> [To Entity] (relationship type) -->
- redis -> pgvector (proxies_from)
- memcached -> pgvector (proxies_to)
- regexextractor -> ollamaextractor (proxies_from)
- pipeline -> regexextractor (proxies_to)
- nginx -> auth-service (proxies_to)
- regexextractor -> pipeline (uses)
- pipeline -> syncsession (executes)
- ollamaextractor -> pipeline (uses)
- pipeline -> confidence (updates)
- pipeline -> source (updates)
- regexextractor -> ollamamodel (configures)
- ollama -> ollamamodel (uses)
- ollama -> ollama.api (implements)
- config.yaml -> yamlonlykeys (defines)
- devlog_core.go -> extraction_log (creates)
- extractor/ollama.go -> ollama (interacts_with)
- devlog_cmds.go -> devlog_sync (defines)
- pipeline -> ollamaextractor (delegates)
- devlog_core.go -> syncsession (uses)
- config/config.go -> entity_extraction (defines)
- devlog_cmds.go -> devlog_sync (executes)
- devlog_cmds.go -> devlog_verify (defines)
- pipeline -> ollamamodel (configures)
- extraction-pipeline -> regexextractor (uses)
- extraction-pipeline -> ollamaextractor (uses)
- ollama -> ollamaextractor (uses)
- internal/extractor/ollama.go -> ollama (interacts_with)
- extraction-pipeline -> ollamaextractor (delegates)
- confidence -> pipeline (updates)
- source -> pipeline (updates)
- ollamaextractor -> ollamamodel (configures)
- ollamaextractor -> ollama (uses)
- devlog_cmds.go -> syncsession (uses)
- ollama -> extraction-pipeline (delegates)
- ollamaextractor -> ollamaextractor (delegates)
- devlogcore.go -> extractionlog (creates)
- extractor.ollama.go -> ollama (interacts_with)
- devlogcmds.go -> devlogsync (defines)
- devlogcore.go -> syncsession (uses)
- configconfig.go -> entityextraction (defines)
- devlogcmds.go -> devlogsync (executes)
- devlogcmds.go -> devlogverify (defines)
- extractionpipeline -> regexextractor (uses)
- extractionpipeline -> ollamaextractor (uses)
- devlogcmds.go -> syncsession (uses)
- ollama -> extractionpipeline (delegates)
- nginx -> authservice (proxies_to)
 
- regex-extractor -> ollama-extractor (uses)
- extraction-pipeline -> ollama-extractor (uses)
- ollama -> ollama-extractor (uses)
- ollama-extractor -> ollama.model (configures)
- ollama-extractor -> ollama (uses)
- devlog_core.go -> extraction-log (creates)
- ollama-extractor -> ollama (delegates)
- ollama -> ollama-model (uses)
- ollama -> ollama-api (implements)
- ollama-extractor -> ollama-model (configures)
- devlog-core -> extraction-log (creates)
- devlog-core -> sync-session (uses)
- devlog_cmds.go -> sync-session (uses)
- ollama -> devlog_cmds.go (delegates)
- ollama -> ollama.model (configures)
- devlog_core -> extraction-log (creates)
- devlog_core -> sync-session (uses)
- devlog_cmds -> sync-session (uses)
- devlog_cmds -> devlog_sync (defines)
- devlog_cmds -> devlog_verify (defines)
